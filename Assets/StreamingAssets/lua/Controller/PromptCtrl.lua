...
function PromptCtrl:Awake(data)
	logWarn("PromptCtrl.Awake--->>");
	panelMgr:CreatePanel('Prompt', PanelForm.NORMAL,this.InitPanel, nil);
end

--启动事件--
function PromptCtrl.InitPanel(obj)
	gameObject = obj;
	...
	resMgr:LoadPrefab('prompt', { 'PromptItem' }, this.InitItem);

    event.AddListener("test", this.test)
end

function PromptCtrl.test(data, data1)
    logError("PromptCtrl >>>>>>>>>>>>event"..data..data1)
end

--初始化面板--
function PromptCtrl.InitItem(objs)
	local count = 100; 
...
function PromptCtrl.OnItemClick(go)
    OpenPanel(PanelNames.Message, go.name)
end
...
function PromptCtrl.OnClick(go)
    ...
    event.RemoveListener("test", this.test)
end
...
function PromptCtrl.Close()
	panelMgr:ClosePanel(PanelNames.Prompt);
end

function PromptCtrl:OnDestroy()
